...
function pair(id: string): string {
  return [
    line({ type: "assistant", uuid: `a-${id}`, parentUuid: null, message: { role: "assistant", content: [{ type: "tool_use", id, name: "Read", input: { file_path: "/x" } }] } }),
    line({ type: "user", uuid: `u-${id}`, parentUuid: `a-${id}`, message: { role: "user", content: [{ type: "tool_result", tool_use_id: id, content: "bytes" }] } }),
  ].join("\n");
}
...
describe("validateTranscript", () => {
  ...
  it("flags an orphaned tool_use (result missing)", () => {
    const raw = line({ type: "assistant", uuid: "a1", parentUuid: null, message: { role: "assistant", content: [{ type: "tool_use", id: "t1", name: "Read", input: {} }] } });
  ...
  it("flags a dangling parentUuid", () => {
    const raw = line({ type: "assistant", uuid: "a1", parentUuid: "ghost", message: { role: "assistant", content: [{ type: "text", text: "hi" }] } });
    const r = validateTranscript(raw);
    expect(r.problems.some((p) => p.code === "dangling-parent")).toBe(true);
  });

  it("flags empty content and bad JSON", () => {
    const raw = ["{ not json", line({ type: "assistant", uuid: "a1", parentUuid: null, message: { role: "assistant", content: [] } })].join("\n");
    const r = validateTranscript(raw);
  ...
  it("newProblems reports only what the fork introduced", () => {
    ...
    const original = validateTranscript(line({ type: "assistant", uuid: "a1", parentUuid: null, message: { role: "assistant", content: [{ type: "tool_use", id: "t1", name: "Read", input: {} }] } }));
    const forked = validateTranscript([
      line({ type: "assistant", uuid: "a1", parentUuid: null, message: { role: "assistant", content: [{ type: "tool_use", id: "t1", name: "Read", input: {} }] } }),
      line({ type: "user", uuid: "u2", parentUuid: "a1", message: { role: "user", content: [{ type: "tool_result", tool_use_id: "t1", content: "" }] } }),
    ].join("\n"));
