...
const COST_PARTS = [
  ...
  { key: "output", color: "#25d07d", pick: (b: SessionDetail["cost"]["breakdown"]) => b.outputUsd },
  {
    key: "cache read",
    color: "#ffa028",
    pick: (b: SessionDetail["cost"]["breakdown"]) => b.cacheReadUsd,
  },
  {
    key: "cache write",
    color: "#b18cf2",
    pick: (b: SessionDetail["cost"]["breakdown"]) => b.cacheWriteUsd,
  },
] as const;
...
export function SessionView({
  ...
  return (
    <div className="grid">
      <div className="pnl area-comp">
        <CompositionPanel xray={d.xray} />
      </div>
      <div className="pnl area-cost">
        <CostPanel cost={d.cost} />
      </div>
      <div className="pnl area-recl">
        <ReclaimablePanel r={d.reclaimable} />
      </div>
      <div className="pnl area-clean">
        <CleanerPanel clean={d.clean} locator={locator} onForked={onForked} />
      </div>
      <div className="pnl area-tape">
        <TapePanel items={d.reclaimable.items} />
      </div>
    </div>
  );
}
                    ...
                    rows={[
                      {
                        sw: x.color,
                        k: money ? "cost" : unit,
                        v: money ? fmtUsd(x.value) : `${fmtInt(x.value)} ${unit}`,
                      },
  ...
  return (
    <>
      <div className="pnl-h">
        ...
        <span className="hint">
          {fmtTokens(xray.totalTokens)} TOK · {xray.segmentCount} SEG
        </span>
      </div>
      <div className="pnl-body">
            ...
            >
              <span
                style={{
                  display: "block",
                  height: "100%",
                  width: `${(c.tokens / total) * 100}%`,
                  background: srcHex(c.source),
                }}
              />
            </Tip>
          ))}
        ...
        <div className="comp-split">
          <div className="donut-wrap">
            <Donut
              data={xray.composition.map((c) => ({
                value: c.tokens,
                color: srcHex(c.source),
                label: c.source,
              }))}
              size={150}
              thick={22}
              ...
              >
                <div className="comp-row">
                  ...
                  <span className="comp-track">
                    <span
                      className="comp-fill"
                      style={{ width: `${(c.tokens / max) * 100}%`, background: srcHex(c.source) }}
                    />
                  </span>
      ...
      <div className="pnl-body">
        <div className="chart-row">
          <div className="donut-wrap">
            <Donut
              money
              data={parts.map((p) => ({ value: p.usd, color: p.color, label: p.key }))}
            />
            <div className="donut-center">
...
function ReclaimablePanel({ r }: { r: SessionDetail["reclaimable"] }) {
  ...
  const pctColor =
    r.reclaimablePct > 0.25 ? "#ff5765" : r.reclaimablePct > 0.1 ? "#ffa028" : "#25d07d";
  return (
    <>
      <div className="pnl-h">
        <span className="bar" />
        <span className="t">Reclaimable</span>
      ...
      <div className="pnl-body">
        <div className="gauge-wrap">
              ...
              <TipChip
                ...
                rows={[
                  ...
                  ...(r.wastedUsdPerTurn != null
                    ? [{ k: "waste/turn", v: fmtUsd(r.wastedUsdPerTurn) }]
                    : []),
                ]}
              />
            }
          ...
          <div className="gauge-info">
            <span className="gauge-big" style={{ color: pctColor }}>
              {fmtPct(r.reclaimablePct)}
            </span>
            <span className="gauge-sub">
              {fmtTokens(r.reclaimableTokens)} of {fmtTokens(r.totalTokens)} tok
            </span>
            {r.wastedUsdPerTurn != null && (
              <span className="gauge-sub" style={{ color: "#ff5765" }}>
              ...
              content={
                <TipChip
                  ...
                  rows={[
                    ...
                    {
                      k: "of reclaimable",
                      v: r.reclaimableTokens ? fmtPct(r.byStatus[s] / r.reclaimableTokens) : "0%",
                    },
                  ]}
                />
              }
            >
              <div className="statbar">
                <span className="statbar-k" style={{ color: STATUS_HEX[s] }}>
                  <span className="d" style={{ background: STATUS_HEX[s] }} />
                  {s}
                </span>
                <span className="statbar-track">
                  <span
                    ...
                    style={{
                      width: `${(r.byStatus[s] / maxStatus) * 100}%`,
                      background: STATUS_HEX[s],
                    }}
                  />
                </span>
                <span className="statbar-v" style={{ color: STATUS_HEX[s] }}>
                  {fmtTokens(r.byStatus[s])}
                </span>
              </div>
            </Tip>
          ))}
...
function CleanerPanel({
  ...
  return (
    <>
      <div className="pnl-h">
        <span className="bar" />
        <span className="t">Context Cleaner</span>
        {!empty && (
          <button
            className={`fork-btn${forking ? " busy" : ""}`}
            type="button"
            onClick={runFork}
            disabled={forking || !locator}
          >
        ...
        <span className="hint">
          {clean
            ? `${clean.summary.copies} COP${clean.summary.copies !== 1 ? "IES" : "Y"} · LOSSLESS`
            : "—"}
        </span>
      </div>
      <div className="pnl-body">
        ...
        {empty ? (
          <div className="clean-empty">
            ✓ NOTHING TO CLEAN — no provable garbage in this session.
          </div>
        ) : (
          <div className="cln">
            {/* summary strip */}
            <div className="cln-kpis">
              <div className="cln-kpi">
                <span className="k">copies</span>
                <span className="v">{fmtInt(clean!.summary.copies)}</span>
              </div>
              <div className="cln-kpi">
                <span className="k">removed</span>
                <span className="v">{fmtTokens(clean!.summary.reclaimableTokens)}</span>
              </div>
              <div className="cln-kpi">
                <span className="k">net reclaimed</span>
                <span className="v" style={{ color: "#25d07d" }}>
                  {fmtTokens(clean!.summary.netReclaimedTokens)}
                </span>
              </div>
            </div>
            ...
            <div className="cln-cmp-chips" style={{ padding: "0 2px 6px" }}>
              {[...byClass.entries()].map(([tag, { count, tokens }]) => (
                <span className="chip" key={tag}>
                  <i style={{ background: EVICT_HEX[tag] ?? "#8a93a6" }} />
                  {tag} {count}× · {fmtTokens(tokens)}
                </span>
              ))}
            </div>
                ...
                return (
                  <Tip
                    ...
                    content={
                      <TipChip
                        ...
                        rows={[
                          {
                            sw: EVICT_HEX[tag] ?? "#8a93a6",
                            k: a.path ?? a.detail,
                            v: fmtTokens(a.reclaimableTokens),
                          },
                        ]}
                      />
                    }
                  >
                    <div className="cln-fix">
                      <span className="act-verb" style={{ color: EVICT_HEX[tag] ?? "#8a93a6" }}>
                        {tag}
                      </span>
...
function TapePanel({ items }: { items: SessionDetail["reclaimable"]["items"] }) {
  return (
    <>
      <div className="pnl-h">
        <span className="bar" />
        <span className="t">Reclaim Tape</span>
            ...
            {items.map((it, i) => (
              ...
              >
                <div className="tape-row">
                  ...
                  <span className="tape-tok" style={{ color: statusColor(it.status) }}>
                    {fmtTokens(it.tokens)}
                  </span>
                </div>
              </Tip>
